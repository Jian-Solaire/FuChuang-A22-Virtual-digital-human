...
class LivePortraitWrapper(object):
    ...
    def prepare_source(self, img: np.ndarray) -> torch.Tensor:
        ...
        # Ensure input_shape values are valid integers to prevent cv2.resize error
        try:
            target_h_raw = self.inference_cfg.input_shape[0]
            target_w_raw = self.inference_cfg.input_shape[1]
            
            # Safely convert to int, handling potential numpy scalars and NaN values
            target_h = int(float(target_h_raw)) if np.isfinite(float(target_h_raw)) else 256
            target_w = int(float(target_w_raw)) if np.isfinite(float(target_w_raw)) else 256
        except (TypeError, ValueError, AttributeError):
            # Fallback to default if conversion fails
            print("Warning: Could not read input_shape from config, using default (256, 256)")
            target_h, target_w = 256, 256
        
        # Verify that target dimensions are positive
        if target_h <= 0 or target_w <= 0:
            raise ValueError(f'Invalid target dimensions: {target_w}x{target_h}')
        
        # Additional check: make sure image dimensions are valid
        if h <= 0 or w <= 0:
            raise ValueError(f'Invalid source image dimensions: {w}x{h}')
        ...
        if h != target_h or w != target_w:
            # Use PIL for resize to completely avoid OpenCV issues
            from PIL import Image
            
            # Convert numpy array to PIL Image
            pil_img = Image.fromarray(img)
            
            # Resize using PIL (more robust)
            resized_pil = pil_img.resize((target_w, target_h), Image.BILINEAR)
            
            # Convert back to numpy array
            x = np.array(resized_pil)
        else:
            x = img.copy()
